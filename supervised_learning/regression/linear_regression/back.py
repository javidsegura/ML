...
"""Introduce any dataset and get inmediate linear regression analysis. This includes
...
when calling 'get_all()' ). They have also been compared with Google spreadsheets, for even 
 more confidence in the prior conclusions"""
...
class Linear_regression: # Superclass with methods for trained models 
      ...
      def get_predictions(self): # Output automatic random predictions to .csv
            ...
            with open(f"/Users/javierdominguezsegura/Programming/Python/Drafts/Scikit/Linear regression/Andrew ng/Multiple linear regression/results/{self.names}results_{title}.csv", "w") as file:
                  writer = csv.writer(file, delimiter= ",")
                  theta_header = [f"θ_{i}" for i in range(len(self.theta))] 
                  headers = theta_header + [i for i in self.names] 
                  writer.writerow(headers)
            with open(f"/Users/javierdominguezsegura/Programming/Python/Drafts/Scikit/Linear regression/Andrew ng/Multiple linear regression/results/{self.names}results_{title}.csv", "a") as file:
                  writer = csv.writer(file, delimiter= ",")
      ...
      def get_all(self, alpha = 0.3, iterations =500): # General function; optional parameters if LSM wants to be modified
            ...
            if isinstance(self, LeastSquaredMethod):
                   ...
                   title = "NormalEquation"
            with open(f"/Users/javierdominguezsegura/Programming/Python/Drafts/Scikit/Linear regression/Andrew ng/Multiple linear regression/results/{self.names}results_{title}.txt","w") as file:
...
class LeastSquaredMethod(Linear_regression):
      ...
      def get_cost_function(self): # Loss function 
            print(self.y_training, self.x_training, self.theta, sep = "\n")
